...
instance MonadUnify TermF Elaborator where
  ...
  equate _ (QuotedType res1 a1) (QuotedType res2 a2) =
    do unless (res1 == res2)
         $ throwError $ "Quoted types have different reset locations: "
             ++ pretty (In (QuotedType res1 a1))
             ++ " and "
             ++ pretty (In (QuotedType res2 a2))
       return [Equation False (instantiate0 a1) (instantiate0 a2)]
  equate _ (Quote m1) (Quote m2) =
    return [Equation False (instantiate0 m1) (instantiate0 m2)]
  equate _ (Unquote m1) (Unquote m2) =
    return [Equation False (instantiate0 m1) (instantiate0 m2)]
  equate _ (Continue m1) (Continue m2) =
    return [Equation False (instantiate0 m1) (instantiate0 m2)]
  equate _ (Shift res1 m1) (Shift res2 m2) =
    do unless (res1 == res2)
         $ throwError "Shifts have different reset locations."
       return [Equation False (instantiate0 m1) (instantiate0 m2)]
  equate _ (Reset res1 m1) (Reset res2 m2) =
    do unless (res1 == res2)
         $ throwError "Resets have different reset locations."
       return [Equation False (instantiate0 m1) (instantiate0 m2)]
  equate _ (Require a1 sc1) (Require a2 sc2) =
    do ns <- freshRelTo (names sc1) context
       let xs = map (Var . Free) ns
       return [ Equation False (instantiate0 a1) (instantiate0 a2)
              , Equation False (instantiate sc1 xs) (instantiate sc2 xs)
              ]
  equate _ l r =
