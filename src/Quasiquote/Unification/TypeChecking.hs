...
import Quasiquote.Unification.Unification ()
...
inferify (In (QuotedType a)) =
  do ElaboratedTerm ela <- checkify (instantiate0 a) (NormalTerm (In Type))
     return (ElaboratedTerm (quotedTypeH ela), In Type)
inferify (In (Quote m)) =
  do (ElaboratedTerm em,t) <- inferify (instantiate0 m)
     return (ElaboratedTerm (quoteH em), quotedTypeH t)
  ...
  case t of
    In (QuotedType a) ->
      do ElaboratedTerm em <-
           addQuoteLevel $ checkify (instantiate0 m)
