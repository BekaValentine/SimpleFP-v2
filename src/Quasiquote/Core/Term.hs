...
quotedTypeH :: Term -> Term
quotedTypeH a = In (QuotedType (scope [] a))
...
instance Parens Term where
  ...
  parenLoc (In (Fun _ _ _)) =
    ...
    ,MotiveRet,ClauseBody,RecFieldType,RecFieldVal
    ]
  parenLoc (In (Lam _ _)) =
    [AnnType,FunArg,FunRet,LamBody,AppArg Impl,ConArg Impl,CaseArg,MotiveArg
    ,MotiveRet,ClauseBody,RecFieldType,RecFieldVal
  ...
  parenLoc (In (Quote _)) =
    [AnnTerm,AnnType,FunArg,FunRet,LamBody,AppFun,AppArg Expl,AppArg Impl
    ,ConArg Expl,ConArg Impl,CaseArg,MotiveArg,MotiveRet,ClauseBody
    ,RecFieldType,RecFieldVal,RecProjArg,QuotedTypeArg
    ]
  ...
  parenRec (In (RecordProj r x)) =
    parenthesize
      ...
      (instantiate0 r)
    ++ "." ++ x
  parenRec (In (QuotedType a)) =
    "Quoted " ++ parenthesize (Just QuotedTypeArg) (instantiate0 a)
  parenRec (In (Quote m)) =
